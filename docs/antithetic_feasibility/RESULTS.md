...
Control fitting, second-moment calibration, Gaussian-table construction and
circuit compilation are classical preprocessing obligations on the quantum
side. Their development timings are retained where measured, but the quoted
quantum execution figures do not add a certified per-price preprocessing or
amortization charge. Readout, classical QAE decoding and output latency also
remain to be charged. They are favorable omissions in a failed screen, never
free resources that could be used to claim a successful crossover. Classical
control fitting is included in the measured comparator.
