...
Implementation correction before any completed pricing case: the first attempt
failed its damping control because Aer `save_probabilities` returned [1,1], while
the saved density matrix for the same circuit was diag(1,0). The retry computes
the objective marginal directly from the saved density-matrix diagonal and
checks normalization/nonnegativity. The gate/channel design is unchanged.
The failed attempt remains in `week3_circuits_v1/failure.json`.
