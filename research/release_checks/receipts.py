...
def junit(path):
    ...
    for suite in suites:
        ...
        actual = {key: sum(c.find(tag) is not None for c in cases)
        ...
        rows.append({'name': suite.attrib.get('name', ''), **counts,
                     'passed': counts['tests']-sum(actual.values())})
    if not any(r['tests'] for r in rows) or any(r['failures'] or r['errors'] or r['skipped'] for r in rows):
