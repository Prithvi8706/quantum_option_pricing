...
REFLECTION = {'representation', 'discount_bridge', 'payoff', 'residual_coefficients',
              'phase_response', 'signal', 'preparation', 'projector', 'radius_bridge',
              'offset', 'beta_rounding', 'decoding_rounding'}
...
def verify_budget(row):
    ...
    if row['mode'] != 'reflection':
        if b['decoder_scale'] != 2*b['beta_upper'] or F(b['decoder_bridge_upper']) > F(parts['decoding']):
