...
def build_report(root=ROOT):
    ...
    for path in paths:
        ...
        required = [
            ...
            + ("run_matched_arithmetic" if matched else "claim_assessment")
            + ".py",
        ]
    ...
    report = {
        ...
        "environment": compare_environment(
            pins(within(root, scope["requirements"]).read_text(encoding="utf-8"))
        ),
        "licenses": license_inventory(root, scope["licenses"]),
        "test_receipts": [
            {"path": name, **junit(within(root, name))} for name in scope["receipts"]
        ],
...
def main():
    ...
    result["tracked_tree_dirty"] = bool(
