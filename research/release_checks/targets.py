...
CASES = [dict(id='D1', split='development', assets=1, dates=2,
              sigma=.2, strike=95., correlation=.3),
         dict(id='D2', split='development', assets=2, dates=2,
              sigma=.25, strike=105., correlation=.4)]
...
def verify_targets(results):
    ...
    for case in cases:
        ...
        if any(plan[k] != v for k, v in
               {'normal_bits': 10, 'fraction_bits': 20, 'width': 40, 'degree': 24}.items()):
