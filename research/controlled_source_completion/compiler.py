...
def resolve_library(manifest,workspace_root=None):
    """Resolve archived libraries after checkout relocation, independent of cwd.

    Historical manifests contain Windows separators and sometimes only the old
    machine's absolute path. Only a results/ suffix can relocate an absolute
    archive path. Explicit existing absolute workspace paths remain supported
    for fresh temporary compilations outside the repository.
    """
    root=Path(workspace_root).resolve() if workspace_root is not None else Path(__file__).resolve().parents[2]
    attempted=[]
    for field in ('library_workspace_relative','library'):
        raw=manifest.get(field)
        if not raw:continue
        normalized=str(raw).replace('\\','/');parts=PurePosixPath(normalized).parts
        if '..' in parts:raise ValueError('Library paths may not traverse parent directories')
        absolute=PurePosixPath(normalized).is_absolute() or bool(PureWindowsPath(str(raw)).drive)
        if absolute:
            marker=next((i for i,p in enumerate(parts) if p.casefold()=='results'),None)
            if marker is not None:candidate=root.joinpath('results',*parts[marker+1:])
            elif field=='library_workspace_relative' and Path(normalized).is_dir():
                return Path(normalized).resolve()
            else:continue
        else:candidate=root.joinpath(*parts)
        attempted.append(str(candidate))
        if candidate.is_dir():return candidate.resolve()
    if attempted:raise FileNotFoundError('Cannot find manifest gate library: '+', '.join(attempted))
    raise ValueError('No workspace-relative gate library or relocatable results/ archive path')
...
def compile_graph(data,path,library=None):
    ...
    library.tables.update(data['tables']);calls=[];sums=Counter();opcost={};maxscratch=0;copies=0;copy_depth=0
    for index,node in enumerate(data['nodes']):
        ...
        copies+=2*arity*w
        copy_depth+=max(Counter(node['args']).values(),default=0)
    ...
    # A repeated SSA argument shares its CNOT controls, requiring one layer per
    # occurrence. Constant leaves have no argument copies. Output aliases need
    # the same treatment when copying into the separate named output registers.
    clean['clifford_t_depth']+=4*copy_depth+max(Counter(data['outputs'].values()).values(),default=0)
