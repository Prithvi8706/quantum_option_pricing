...
def rate_interval(successes, trials):
    ...
    return [
        ...
        float(beta.ppf(0.975, successes + 1, trials - successes))
...
def summarize_cell(rows):
    ...
    return {
        "attempts": len(rows), "executed": len(executed), "nonempty": len(nonempty),
        "declarations": declarations, "false_declarations": false,
...
def main():
    ...
    for name in ("price_intervals_v2c", "pricing_gate_v2b", "pricing_gate_v2a_retry1"):
        ...
        hashes[name] = {"artifacts_verified": len(complete["sha256"]),
    ...
    write_json(path / "verification.json", {
        "input_hashes": hashes, "raw_sha256": sha256(raw), "rows": len(rows),
        "cells": len(cells), "states": dict(Counter(r["state"] for r in rows)),
        "reinversion_and_analysis_seconds": time.perf_counter() - start,
        "bound_construction_72_candidates_seconds": construction,
        "timing_note": "Separate post-run CPU timing; not full circuit setup or quantum runtime",
        "all_checks_pass": True,
    })
    commands = [
        ["git", "log", "--all", "--oneline", "--", "src/plot_dimension_sweep.py"],
        ["git", "log", "--all", "--name-only", "--format=", "--",
         "data", "results", "outputs", "figures", "src", "research"],
    ]
    evidence = []
    for command in commands:
        ...
        if "--name-only" in command:
            lines = [line for line in lines if any(s in line.lower() for s in
                     ("dimension", "sweep", "trial", "rqmc", ".npz", ".csv"))]
