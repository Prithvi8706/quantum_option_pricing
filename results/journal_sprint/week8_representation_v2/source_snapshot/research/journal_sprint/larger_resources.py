...
def resource_matrix(profile):
    if profile not in {"week3", "week5-k2"}:
    ...
    return [
        ...
        for n in (5, 6)
        for k in ((0, 1) if profile == "week3" else (2,))
    ]
...
def main(argv=None):
    ...
    parser.add_argument("--profile", choices=["week3", "week5-k2"], default="week3")
    args = parser.parse_args(argv)
    matrix = resource_matrix(args.profile)
    protocol = "PROTOCOL_W3_RESOURCES" if args.profile == "week3" else "PROTOCOL_W5_RESOURCES"
    output = args.output or (
        "results/journal_sprint/week3_resources_v1"
        if args.profile == "week3"
        else "results/journal_sprint/week5_resources_v1"
    )
    path = start_run(
        output,
        {
    ...
    sources += [ROOT / f"docs/journal_sprint/{protocol}.md"]
    ...
    try:
        for cid, n, scale, k in matrix:
            ...
            preparation_and_compile_seconds = time.perf_counter() - tick
            ...
            row = {
                ...
                "preparation_and_compile_seconds": preparation_and_compile_seconds,
