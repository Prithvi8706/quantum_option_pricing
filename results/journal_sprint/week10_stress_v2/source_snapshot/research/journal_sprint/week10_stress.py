...
def cases(extension=False):
    ...
    for ds in schedules:
        for n in (128, 30000, 100000):
            for guard in ('0', '.01'):
                for counts in ([0]*len(ds), [n]*len(ds),
...
def reference(row, precision):
    with mp.workdps(precision):
        ...
        for count, depth in zip(row['counts'], row['depths']):
            ...
            branch = nr.preimage(max(0, (lo-upper)/(1-upper)), min(1, hi/(1-upper)), depth)
            result = nr.union([(max(a,c), min(b,d)) for a,b in result for c,d in branch])
        return result


def compare(row):
    production, _ = invert_calibrated(row['counts'], [row['shots']]*len(row['depths']),
        row['depths'], [0,0], [64,64], transfer_bounds=(float(row['guard']),)*2)
    a, b = reference(row, 80), reference(row, 100)
    with mp.workdps(100):
        stable = len(a) == len(b) and all(abs(x-y) < mp.mpf('1e-40')
            for pair, other in zip(a,b) for x,y in zip(pair,other))
        enclosed = all(any(mp.mpf(c) <= lo and hi <= mp.mpf(d)
            for c,d in production.components) for lo,hi in b)
        return dict(**row, stable=stable, encloses=enclosed,
...
def main():
    ...
    sources = [ROOT/'research/journal_sprint'/name for name in
        ('week10_stress.py','numerical_reference.py','calibrated_readout.py','intervals.py','storage.py')]
    ...
    output = start_run(args.output, dict(protocol='PROTOCOL_W10_STRESS', extension=args.extension, mpmath=mp.__version__,
    ...
    try:
        ...
        summary = dict(cases=len(rows), failed=sum(not r['encloses'] for r in rows),
            unstable=sum(not r['stable'] for r in rows),
            disconnected=sum(len(r['reference'])>1 for r in rows),
            disconnected_intersections=sum(r['depths']==[1,2] and len(r['reference'])>1 for r in rows),
            empty=sum(not r['reference'] for r in rows), elapsed=time.monotonic()-tick)
