...
def main():
    ...
    path = start_run("results/journal_sprint/week3_circuit_analysis_v1", {
    ...
    for row in rows:
        for kind, noise in row["noise"].items():
            ...
            diagnostics.append({
                "id": row["id"], "channel": kind, "probability": value,
                "ideal": row["ideal"], "allowed_symmetric_range": [lower, upper],
    ...
    write_json(path / "verification.json", {
        "artifacts_verified": len(manifest["sha256"]), "events_verified": len(events),
        "max_ideal_difference": summary["max_ideal_difference"],
        "outside_symmetric_response": {
            kind: sum(d["outside_any_eta_response"] for d in diagnostics if d["channel"] == kind)
            for kind in ("depolarizing", "amplitude_damping")
        },
        "environment": {name: importlib.metadata.version(name) for name in
                        ("qiskit-terra", "qiskit-aer", "qiskit-finance", "numpy", "scipy")},
