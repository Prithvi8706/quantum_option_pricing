...
def main():
    ...
    for name, digest in config["dependency_sha256"].items():
        require(
            sha256(snap / name) == digest and sha256(ROOT / name) == digest, f"source drift: {name}"
        )
    cases = case_table()
    ...
    for name, attempts in (
        ...
        evidence.append(
            dict(
                ...
                scope="manifest integrity and saved-summary aggregation, not a fresh full pricing replay",
            )
        )
    ...
    write_json(
        ...
        dict(
            ...
            **expected,
            scope="repeat of independently implemented high-precision reference, not formal certification",
        ),
    )
