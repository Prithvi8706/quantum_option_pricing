...
def main():
    ...
    runs = [
        ...
        "comparator_v1",
        "week3_clean_circuits_v1",
        "week3_bae_smoke_v2",
        "week3_biqae_smoke_v2",
    ...
    original = json.loads((base / "week3_circuits_v1_retry1/summary.json").read_text())["rows"]
    replay = json.loads((base / "week3_clean_circuits_v1/summary.json").read_text())["rows"]
    if len(original) != len(replay) or len(replay) != 24:
        raise RuntimeError("clean replay case count mismatch")
    for old, new in zip(original, replay):
        if any(old[k] != new[k] for k in ("id", "gates", "depth", "total_qubits")):
            raise RuntimeError("clean replay resource mismatch")
        if abs(old["ideal"] - new["ideal"]) > 1e-9:
            raise RuntimeError("clean replay ideal mismatch")
        for channel in old["noise"]:
            if (
                abs(old["noise"][channel]["probability"] - new["noise"][channel]["probability"])
                > 1e-9
            ):
                raise RuntimeError("clean replay noise mismatch")
    ...
    write_json(
        path / "summary.json",
        {
            ...
            "clean_replay_cases_verified": len(replay),
