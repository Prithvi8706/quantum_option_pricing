...
def validate_config(planned, manifests):
    expected = {
    ...
    require(planned == expected, "declared configuration differs from reconstruction")


def validate_sources(source):
    snapshot = source / "source_snapshot"
    files = list(snapshot.rglob("*.py"))
    require(bool(files), "missing source snapshot")
    for archived in files:
        current = ROOT / archived.relative_to(snapshot)
        require(
            current.is_file() and sha256(current) == sha256(archived),
            f"live source differs from archive: {current}",
        )
    for name in (
        "run_fixed_discovery",
        "calibrated_readout",
        ...
        require(
            (snapshot / f"research/journal_sprint/{name}.py").is_file(),
            f"missing helper snapshot: {name}",
        )
